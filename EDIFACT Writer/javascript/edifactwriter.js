/*
 * IPWorks EDI 2022 JavaScript Edition - Demo Application
 *
...
async function main() {
    ...
    edifactwriter1 = new ipworksedi.edifactwriter();

    await edifactwriter1.config("Encoding=ISO-8859-1");
    edifactwriter1.setSchemaFormat(6, function (err) { //schemaJSON = 6
        if (err) {
            console.log(err);
            return;
        }
    ...
    await edifactwriter1.loadSchema('./RSSBus_D97A_' + argv[2] + '.json');
...
async function writeFile_EDIFACT_DESADV() {
    ...
    edifactwriter1.createInterchangeFooter();
}
